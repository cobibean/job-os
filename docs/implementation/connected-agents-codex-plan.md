...
- **Status:** Active implementation — Phases 0–7 delivered; Phase 8 in progress
...
- **Progress update (2026-08-25):** Phases 0–7 are merged and verified. Phase 6 shipped the Connected Agents roster/inspector, live provider model defaults, device-code and disconnect flows, immutable New Chat selection, readable locked history, and five-chat archive recovery in [PR #127](https://github.com/cobibean/job-os/pull/127). Phase 7 shipped scoped concurrency/failure/recovery behavior, honest remote-host and authentication states, bounded credential-free diagnostics, and terminal Codex rate-limit handling that does not blindly retry or switch providers in [PR #128](https://github.com/cobibean/job-os/pull/128); its closeout evidence is recorded on [#118](https://github.com/cobibean/job-os/issues/118#issuecomment-5407647236). Phase 8 now builds and verifies an arm64 package containing the exact pinned Codex runtime, license/notice receipts, valid signatures, a JobOS-owned Codex home, and launchd/MCP wiring that retrieves credentials from macOS Keychain without credential files or token-bearing app-server environment. The 62-ID evidence index records prior immutable proof and leaves incomplete installed/human criteria visibly unresolved. Real installed-data upgrade, live device-code/Keychain authentication, MacBook/Tailscale operation, useful-work mutation, and final human visual/accessibility acceptance remain behind their explicit fresh approvals.
