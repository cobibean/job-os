...
import type { AgentConversationSnapshot, AgentSessionStreamUpdate, ConversationEvent } from '../../shared/contracts'
...
type UnscopedUpdate = AgentSessionStreamUpdate extends infer Update
  ? Update extends { conversationId: string } ? Omit<Update, 'conversationId'> : never
  : never
...
function installAgent(
  ...
  const suppliedSubscribe = overrides.subscribe as ((listener: (update: UnscopedUpdate) => void) => () => void) | undefined
  const conversationId = snapshot.conversationId || CONVERSATION_ID
  const position = snapshot.position ?? 1
  const agent = {
    list: vi.fn().mockResolvedValue([{
      conversationId, position, title: snapshot.title ?? 'Session 1', createdAt: snapshot.createdAt ?? '',
      activeTurn: snapshot.activeTurn ?? null, connection: snapshot.connection ?? 'online', latestEventId: snapshot.latestEventId ?? 0
    }]),
    create: vi.fn(), archive: vi.fn().mockResolvedValue(undefined),
    get: vi.fn().mockResolvedValue({ ...snapshot, conversationId, position }),
    ...
    ...overrides,
    subscribe: vi.fn((listener: (update: AgentSessionStreamUpdate) => void) => suppliedSubscribe
      ? suppliedSubscribe(update => listener({ ...update, conversationId } as AgentSessionStreamUpdate))
      : () => undefined)
  }
...
test('groups a completed turn, starts collapsed, and keeps the final answer last', async () => {
  ...
  render(<Harness apiState="connected" contextLabel="Northstar · Staff PM" />)
...
test.each([
  ...
  await waitFor(() => expect(agent.retry).toHaveBeenCalledWith(CONVERSATION_ID, '', expect.stringMatching(/^desktop-retry/)))
})
...
test('follows the bottom until the user detaches, then offers Jump to latest', async () => {
  let resolveSnapshot!: (snapshot: Partial<AgentConversationSnapshot>) => void
  let stream!: (update: unknown) => void
  installAgent({} as AgentConversationSnapshot, {
    get: vi.fn(() => new Promise<Partial<AgentConversationSnapshot>>(resolve => { resolveSnapshot = resolve })),
    subscribe: vi.fn((listener: (update: unknown) => void) => { stream = listener; return () => undefined })
  })
  render(<Harness apiState="connected" contextLabel="Northstar" />)
  const transcript = await screen.findByRole('tabpanel')
...
test('composer supports Enter submission, Shift+Enter drafting, and preserves a draft across job changes', async () => {
  ...
  const { rerender } = render(<Harness apiState="connected" contextLabel="Northstar · Staff PM" />)
  ...
  rerender(<Harness apiState="connected" contextLabel="Daybreak · Platform PM" />)
  ...
  await waitFor(() => expect(agent.send).toHaveBeenCalledWith(CONVERSATION_ID, 'Draft for this conversation', expect.stringMatching(/^desktop-/)))
})
...
test('running and waiting turns keep drafting enabled while submission is serialized and Stop remains available', async () => {
  ...
  render(<Harness apiState="connected" contextLabel="No active job" />)
  ...
  expect(agent.cancel).toHaveBeenCalledWith(CONVERSATION_ID, 'turn-1')
...
test('failed and interrupted turns expose Retry and offline states remain distinct', async () => {
  ...
  const { rerender } = render(<Harness apiState="connected" contextLabel="Northstar" />)
...
test('focuses the document surface when a live resume render completes', async () => {
  ...
  render(<Harness apiState="connected" contextLabel="Northstar" onArtifactRendered={onArtifactRendered} />)
...
test('restoring and reconnecting states are calm and announced without exposing token-level detail', async () => {
  let resolve!: (snapshot: Partial<AgentConversationSnapshot>) => void
  installAgent({} as AgentConversationSnapshot, {
    get: vi.fn().mockReturnValue(new Promise<Partial<AgentConversationSnapshot>>(value => { resolve = value })),
