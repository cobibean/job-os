...
import { exportEditableDocumentDocx } from './export/documentDocx.js'
import { exportEditableDocumentPdf } from './export/pdfExporter.js'
import { importDocx } from './import/docxImporter.js'
import { loadVerifiedArtifactBytes } from '../artifacts/documents.js'
...
async function apiJson<T>(config: DesktopApiConfig, route: string, init: RequestInit = {}): Promise<T> {
...
async function loadEditableDocument(config: DesktopApiConfig, documentId: string): Promise<EditableDocument> {
  return toDocument(await apiJson<ApiDocument>(
    config,
