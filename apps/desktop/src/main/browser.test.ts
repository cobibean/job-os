...
test('agent browser operations paginate long page text with bounded overlapping semantic snapshots', async () => {
  ...
  expect(snapshotScript).toContain("querySelectorAll('iframe,frame')")
  expect(snapshotScript).toContain('frame.contentDocument')
  expect(snapshotScript).toContain("frameTexts.join('\\n\\n')")
